...
local function createActionButton(parent, text, func)
    ...
    -- Клик колесиком (средняя кнопка) – начать назначение бинда
...
-- Воспроизведение песни "Человек яйца" по заданному ID
...
-- Peredoz: радужный экран 1 минута + песня
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if listeningForBind then
        ...
        -- Проверяем, что это клавиша клавиатуры
        if input.UserInputType == Enum.UserInputType.Keyboard then
            ...
            if key ~= Enum.KeyCode.Unknown then
                ...
                -- Удаляем предыдущую привязку этой функции (если была)
                for k, f in pairs(keyToFunction) do
                ...
                -- Обновляем текущую вкладку, чтобы кнопки показали новый бинд
...
print("Black Flash Helper загружен! Нажмите P, чтобы скрыть/показать. Используйте КОЛЕСИКО МЫШИ по кнопке для назначения бинда.")

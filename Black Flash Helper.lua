...
local function playEggManSong()
    ...
    sound.Volume = 0.5
    sound.Looped = true  -- зацикливаем на 1 минуту
    sound.Parent = player.PlayerGui  -- звук будет жить, пока жив PlayerGui
...
-- Peredoz: радужный экран 1 минута + песня "Человек яйца"
local peredozActive = false
local rainbowFrame = nil

local function peredoz()
    -- Эффект падения (для атмосферы)
    local localPlayer = game.Players.LocalPlayer
    local character = localPlayer.Character
    if character then
    ...
    -- Создаём полупрозрачный полноэкранный фрейм для радуги
    ...
    -- Анимация радуги (быстрая смена цветов)
    local hue = 0
    local startTime = tick()
    local duration = 60 -- секунд
    local connection
    ...
    connection = runService.RenderStepped:Connect(function()
        ...
        -- Плавно меняем цвет по кругу (радуга)
        hue = hue + 0.01

...
-- Peredoz: быстрое мигание чёрный/белый/оранжевый в течение 30 секунд
local peredozActive = false
local flashFrame = nil

local function peredoz()
    -- Эффект падения (можно оставить для атмосферы)
    local localPlayer = game.Players.LocalPlayer
    local character = localPlayer.Character
    if character then
    ...
    flashFrame.BackgroundColor3 = Color3.new(0, 0, 0) -- начнём с чёрного
    flashFrame.BackgroundTransparency = 0.3 -- полупрозрачный, чтобы видеть игру
    flashFrame.BorderSizePixel = 0
    flashFrame.ZIndex = 10
    flashFrame.Parent = player:WaitForChild("PlayerGui")

    -- Цвета для мигания
    local colors = {
        Color3.new(0, 0, 0),       -- чёрный
        Color3.new(1, 1, 1),       -- белый
        Color3.fromRGB(255, 165, 0) -- оранжевый
    }

    local startTime = tick()
    local duration = 30 -- секунд
    local flashInterval = 0.1 -- меняем цвет каждые 0.1 сек
    ...
    connection = runService.Heartbeat:Connect(function()
        ...
        local index = math.floor(elapsed / flashInterval) % #colors + 1

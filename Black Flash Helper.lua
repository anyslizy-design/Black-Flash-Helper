...
screenGui.Enabled = true -- по умолчанию включено
...
local function startRainbowEffect(duration)
    -- Если эффект уже активен, останавливаем его и запускаем заново
    ...
    effectGui.DisplayOrder = 999 -- поверх всего
    ...
    rainbowFrame.BackgroundTransparency = 0.3 -- небольшая прозрачность, чтобы видеть игру
    ...
    -- Автоматически отключаем через duration секунд
...
local function peredoz()
    startRainbowEffect(5) -- эффект длится 5 секунд

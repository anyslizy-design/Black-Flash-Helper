...
animeImage.Size = UDim2.new(1, -20, 0, 120)  -- высота 120, ширина под отступы
animeImage.Position = UDim2.new(0, 10, 1, -130) -- прижато к низу с отступом
animeImage.BackgroundTransparency = 1
animeImage.Image = "rbxassetid://6023426918"  -- пример аниме-аватарки (замени на свой ID)
...
local function updateButtonText(button, baseText, bindKey)
    if bindKey and bindKey ~= Enum.KeyCode.Unknown then
        -- Получаем название клавиши (например, "E", "LeftShift")
...
-- Система биндов
local bindings = {}  -- ключ: кнопка (объект), значение: KeyCode
...
local function onInputBegan(input, gameProcessed)
    ...
    if bindingMode and currentBindingButton then
        -- Если нажата любая клавиша (кроме мыши? можно и мышь, но обычно клавиши)
        if input.UserInputType == Enum.UserInputType.Keyboard then
            ...
            if key ~= Enum.KeyCode.Unknown then
                ...
                local baseText = currentBindingButton.BaseText  -- сохраним базовый текст в самой кнопке
                updateButtonText(currentBindingButton, baseText, key)
                bindingMode = false
                currentBindingButton = nil
            end
        elseif input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.MouseButton2 then
            -- Игнорируем мышь для биндов (можно разрешить, но обычно нет)
            -- Если хотим разрешить, можно добавить
        end
        return
    end

    -- Если не в режиме бинда, проверяем, не нажата ли забинженная клавиша
...
local function createFunctionButton(parent, name, baseText, posY, callback)
    ...
    -- Храним базовый текст в кнопке
    button.BaseText = baseText

    -- Обработка обычного клика
    button.MouseButton1Click:Connect(callback)

    -- Обработка ПКМ для назначения бинда
    button.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton2 then
            -- Предотвращаем контекстное меню
            input:Consume()
            -- Запускаем режим бинда
            bindingMode = true
            currentBindingButton = button
            -- Временно меняем текст
            button.Text = "Press any key..."
            -- Можно добавить таймаут на случай, если пользователь передумает
